...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::IAN)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_IanCoin4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_IanCoin, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("IanCoin")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  ui->label_mcolat->setText("1000000 Coins");
  ui->label_fcolat->setText("5000000 Coins");
...
double BlockRewardIanCoin =  static_cast<double>(BlockReward/COIN);
//int64_t IanCoinSupply = chainActive.Tip()->nMoneySupply / COIN;
...
ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardIanCoin));
//ui->label_IanCoinSupply_value->setText(QString::number(IanCoinSupply));
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeIanAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIanAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeIanAmount = strAnonymizeIanAmount.remove(strAnonymizeIanAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIanAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeIanAmount * COIN) nMaxToAnonymize = nAnonymizeIanAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeIanAmount * COIN) {
        // ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
        //                                       .arg(strAnonymizeIanAmount));
        strAnonymizeIanAmount = strAnonymizeIanAmount.remove(strAnonymizeIanAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIanAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
